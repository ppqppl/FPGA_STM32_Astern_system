...
def udp_send(disstr):
    ...
    # 30000  表示目的地端口
    udp_socket1.sendto(disstr.encode("utf-8"), ("192.168.0.10", 6000))
            ...
            while if_com_open:
                if custom_serial.in_waiting:  # 如果串口接收到了数据
                    ...
                    if num_all <= 7:
                        ...
                        print(char)
                        distance = float(char)
                        udp_send(str(distance))
                        # disstr = char
                        # udp_send(disstr)
                        # print(char[0:7])
                        print("距离：", distance, "cm")
                        window.fpga_com_display(str(distance))
                        char = ""
                        num_all = 0
        else :
            while if_com_open:
                ...
                if data == b'':
                    ...
                    # char = str(data)
                    char = str(data, 'utf-8', errors='ignore')
                    print("com:  ", data)
...
class Mywindow(QtWidgets.QMainWindow, main.Ui_MainWindow):
    def __init__(self):
        ...
        # 设置窗口名称
        self.setWindowTitle('ppqppl——调试助手')
